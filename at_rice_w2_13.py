...
def hierarchical_clustering(cluster_list, num_clusters):
    '''
    hierarchicaol to get the clusters
    first initial num_clusters clusters
    then find closest pair until it fits the sizes
    '''
    length = len(cluster_list)
    while length > num_clusters:
        dummy_distance, idx_i, idx_j = fast_closest_pair(cluster_list)
...
def kmeans_clustering(cluster_list, num_clusters, num_iterations):
    '''
    through iterations to find the center of some clusters
    then find the nearest center of it and again again
    '''
    ...
    for dummy_itr in range(num_iterations):
        cluster = []
        for dummy_num in range(num_clusters):
